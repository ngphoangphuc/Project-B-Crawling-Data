...
sql_business = "INSERT INTO businessresult (CompanyID, Time,Net_Sales_And_Service_Provision," \
                   "Cost_Of_Goods_Sold, Gross_Profit_On_Sales_And_Service_Delivery," \
                   "Financial_Operating_Revenue,Financial_Expenses," \
                   "Cost_Of_Sales,Business_Management_Expenses,Net_Profit_From_Business_Activities," \
                   "Other_Profits, Profit_Or_Loss_Portion_From_The_Joint_Venture_Affiliate," \
                   "Total_Accounting_Profit_Before_Tax," \
                   "Profit_After_Corporate_Income_Tax," \
                   "After_Tax_Profit_Of_Shareholders_Of_The_Parent_Company," \
                   "Underlying_Earnings_Per_Share) " \
...
def check_data(all_data,code):
    ...
    mycursor.execute(f"SELECT * FROM vietstock2.businessresult where CompanyID='{code}'")
    ...
    for datas in all_data:
        ...
        for result in results:
            ...
            if datas[1] == result[2] and  (str(datas[3]) == str(result[4]) or str(datas[3]) == str(result[4]).split('.')[0]):
                check = True
                old += 1
                break
            # match only date
            elif datas[1] == result[2]:
                check = True
                update += 1
                listA = [result[0]]
...
class Worker_businessResult(QObject):
