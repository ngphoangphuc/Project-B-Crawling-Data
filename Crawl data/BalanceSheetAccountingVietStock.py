...
sql_balancesheet = "INSERT INTO balancesheetaccounting (CompanyID,Source, Time, Short_Term_Assets," \
                   "Money_And_Cash_Equivalents, Short_Term_Financial_Investments, Short_Term_Receivables," \
                   "Inventory, Other_Short_Term_Assets, Long_Term_Assets,Fixed_Assets, Investment_Real_Estate," \
                   "Long_Term_Financial_Investments, Total_Assets, Liabilities, Short_Term_Debt," \
                   "Long_Term_Debt, Equity," \
                   "Investment_Capital_Of_Owner, Equity_Surplus, Undistributed_After_Tax_Profit," \
                   "Benefits_Of_Minority_Shareholders, Total_Capital) " \
...
def check_data(all_data,code):
    ...
    mycursor.execute(f"SELECT * FROM vietstock2.balancesheetaccounting where CompanyID='{code}'")
...
def get_year_datas(code):
    ...
    if  len(all_data_2V2[0]) == 23:
        new_data, update_data, old, update = check_data(all_data_2V2, code)

        insert_sql(sql_balancesheet, new_data)
...
class Worker_balanceSheet(QObject):
